...
export const apiCrudFactory = ({api, defaultFilter, getRelationRoute}) => (path, {transformers, urls} = {}) => {
  path = path.replace('/', '')
  return {
    ...
    removeRelation: (itemId, {location}) => {
      ...
      return api.patch(`${path}/${id}/${relationRoute || relation}/${itemId}`, {removed: true})
    },
    ...
    undoRemoveRelation  : (itemId, {location}) => {
      const relationRoute = getRelationRoute && getRelationRoute(location)
      const [relation, id] = location.pathname.split('/').reverse()
      return api.patch(`${path}/${id}/${relationRoute || relation}/${itemId}`, {removed: false})
    },
    getRelationList: ({location, ...filter}) => {
      const relationRoute = getRelationRoute && getRelationRoute(location)
      const [relation, id] = location.pathname.split('/').reverse()
      return getList(api, `${path}/${id}/${relationRoute || relation}`, defaultFilter)(filter)
    },
    getRelationCount: ({location, filter}) => {
      const relationRoute = getRelationRoute && getRelationRoute(location)
      const [relation, id] = location.pathname.split('/').reverse()
      return api.get(`${path}/${id}/${relationRoute || relation}/count`, R.mergeDeepWith(R.join, defaultFilter, filter))
    }
  }
}
